...
static sdpplin_stream_t *sdpplin_parse_stream(char **data) {
  ...
  *data=nl(*data);
  ...
  if (!got_mimetype) {
    printf("libreal: sdpplin_stream: no mimetype\n");
    desc->mime_type = strdup("audio/x-pn-realaudio");
    desc->mime_type_size = strlen(desc->mime_type);
    if (desc->stream_id)
      printf("libreal: sdpplin_stream: implicit mimetype for stream_id != 0, weird.\n");
  }
...
sdpplin_t *sdpplin_parse(char *data) {
  ...
  while (data && *data) {
    ...
    if (filter(data, "m=", &buf)) {
      ...
      {
      printf("sdpplin: bad stream_id %d (must be >= 0, < %d). Broken sdp?\n",
        stream->stream_id, desc->stream_count);
      free(stream);
      } else {
        printf("sdpplin: got 'm=', but 'a=StreamCount' is still unknown.\n");
        if (stream->stream_id == 0) {
          desc->stream_count=1;
          desc->stream=malloc(sizeof(sdpplin_stream_t*));
          desc->stream[0]=stream;
        } else {
          printf("sdpplin: got 'm=', but 'a=StreamCount' is still unknown and stream_id != 0. Broken sdp?\n");
          free(stream);
        }
      }
      continue;

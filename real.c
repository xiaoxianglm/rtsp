...
static int select_mlti_data(const char *mlti_chunk, int mlti_size, int selection, char **out) {
  ...
  if (codec >= numrules) {
    printf("realrtsp: codec index >= number of codecs. %i %i\n",codec, numrules);
    return 0;
  }
...
int real_get_rdt_chunk(rtsp_t *rtsp_session, char **buffer, int rdt_rawdata) {
  ...
  if (header[0] != 0x24)
  {
    printf("realrtsp: rdt chunk not recognized: got 0x%02x\n",
      header[0]);
    return 0;
  }
  ...
  if ((flags1 & 0xc0) != 0x40)
  {
    ...
    if(header[6] == 0x06) { // eof packet
      ...
      if(flags1 & 0x7c) // ignore eof for streams with id != 0
        return 0;
      printf("realrtsp: Stream EOF detected\n");
      return -1;
    }
    header[0]=header[5];
    header[1]=header[6];
...
rtsp_send_describe:
  ...
  if (status == 401) {
    ...
    if (authfield) {
      printf("realrtsp: authorization failed, check your credentials\n");
      goto autherr;
    }
...
autherr:
  ...
  if ( status<200 || status>299 )
  {
    ...
    if (alert) {
      printf("realrtsp: got message from server:\n%s\n",
        alert);
    }
  ...
  if (size > MAX_DESC_BUF) {
    printf("realrtsp: Content-length for description too big (> %uMB)!\n",
            MAX_DESC_BUF/(1024*1024) );
    goto out;
  }

  if (!rtsp_search_answers(rtsp_session,"ETag"))
    printf("realrtsp: got no ETag!\n");
  else

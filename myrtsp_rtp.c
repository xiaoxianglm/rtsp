...
static void rtp_session_set_fd (struct rtp_rtsp_session_t *st,
...
static int parse_port (const char *line, const char *param,
...
static char * parse_destination (const char *line)
{
  ...
  strlcpy (dest, parse1 + strlen (RTSP_SETUP_DESTINATION), len);
  free (line_copy);
...
static int rtp_connect (char *hostname, int port)
{
  ...
  if (setsockopt (s, SOL_SOCKET, SO_RCVBUF,
    ...
    printf("Couldn't set receive socket buffer size\n");
...
struct rtp_rtsp_session_t * rtp_setup_and_play (rtsp_t *rtsp_session,const char *user,const char *pass)
{
  ...
  if(401 == statut)
  {
	...
	if (!(authreq = rtsp_search_answers(rtsp_session,"WWW-Authenticate: Digest"))) {
	  printf("realrtsp: 401 but no auth request, aborting\n");
	  return NULL;
	}
	if (!user) {
	  printf("realrtsp: auth required but no username supplied\n");
	  return NULL;
	}
	if (!(authreq = rtsp_search_answers(rtsp_session,"WWW-Authenticate: Basic"))) {
	  printf("realrtsp: authenticator not supported (%s)\n", authreq);
	  return NULL;
	}
	authlen = strlen(user) + 1 + (pass ? strlen(pass) : 0);
	authstr = malloc(authlen + 1);
	sprintf(authstr, "%s:%s", user, pass ? pass : "");
	b64_authlen = BASE64_SIZE(authlen);
	authfield = malloc(21 + b64_authlen);
	strcpy(authfield, "Authorization: Basic ");
	base64_encode(authfield + 21, b64_authlen, authstr, authlen);
	free(authstr);
  ...
  {
    printf("A single media stream only is supported atm. %d\n",fsdp_get_media_count(dsc));
    fsdp_description_delete (dsc);
    return NULL;
  }
*/
  ...
  if (!med_dsc)
  {
  	printf("check for a valid media stream\n");
    fsdp_description_delete (dsc);
    return NULL;
  }
      ...
      fsdp_get_media_type (med_dsc) != FSDP_MEDIA_AUDIO)
  {
 	...
 	printf("parse the `m=<media>  <port>  <transport> <fmt list>' line\n");
    fsdp_description_delete (dsc);
    return NULL;
  }
  ...
  {
	printf("only RTP/AVP transport method is supported right now\n");
    fsdp_description_delete (dsc);
    return NULL;
  }
  ...
  {
    printf("only MPEG-TS is supported at the moment\n");
    fsdp_description_delete (dsc);
    return NULL;
  }
*/
  ...
  {
    printf("get client port (if any) advised by server\n");
    fsdp_description_delete (dsc);
    return NULL;
  }
  ...
  printf("RTP Port from SDP appears to be: %d\n", client_rtp_port);
  printf("RTCP Port from SDP appears to be: %d\n", client_rtcp_port);
  ...
  printf( "RTSP Destination: %s\n", destination);
  printf( "Client RTP port : %d\n", client_rtp_port);
  printf( "Client RTCP port : %d\n", client_rtcp_port);
  printf( "Server RTP port : %d\n", server_rtp_port);
  printf( "Server RTCP port : %d\n", server_rtcp_port);
  ...
  /*
  if(strstr(destination,"0.0.0.0"));
  {
	sprintf(destination,"%s",rtsp_session->host);
  }

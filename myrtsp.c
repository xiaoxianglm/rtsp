...
int read_streamdata(int fd, void *buf, int count) {
  ...
  do { /* while we get answer lines */
    ...
    if (!strncasecmp(answer,"Session:",8)) {
      ...
      } else
      {
        ...
        s->session=strdup(buf);
		if(pstime = strstr(buf,"timeout="))
		{
			sscanf(strstr(pstime,"=")+1,"%u",&timeout);
			if (s->timeout != timeout) {
			  printf("librtsp: warning: timeout mismatch. got %d,  %d\n", timeout, s->timeout);
			  s->timeout=timeout;
			}
		}
		else
